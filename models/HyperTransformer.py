...
class VGG_LFE(torch.nn.Module):
    def __init__(self, in_channels, requires_grad=True, rgb_range=1):
        ...
        self.conv_RGB = nn.Conv2d(
            in_channels=in_channels, out_channels=3, kernel_size=3, padding=1)
...
class LFE_lvx(nn.Module):
    def __init__(self, in_channels, n_feates, level):
        ...
        # First level convolutions
        self.conv_64_1 = nn.Conv2d(
            in_channels=self.in_channels, out_channels=lv3_c, kernel_size=7, padding=3)
        self.bn_64_1 = nn.BatchNorm2d(lv3_c)
        self.conv_64_2 = nn.Conv2d(
            in_channels=lv3_c, out_channels=lv3_c, kernel_size=3, padding=1)
        self.bn_64_2 = nn.BatchNorm2d(lv3_c)

        # Second level convolutions
        if self.level == 1 or self.level == 2:
            self.conv_128_1 = nn.Conv2d(
                in_channels=lv3_c, out_channels=lv2_c, kernel_size=3, padding=1)
            self.bn_128_1 = nn.BatchNorm2d(lv2_c)
            self.conv_128_2 = nn.Conv2d(
                in_channels=lv2_c, out_channels=lv2_c, kernel_size=3, padding=1)
            self.bn_128_2 = nn.BatchNorm2d(lv2_c)

        # Third level convolutions
        if self.level == 1:
            self.conv_256_1 = nn.Conv2d(
                in_channels=lv2_c, out_channels=lv1_c, kernel_size=3, padding=1)
            self.bn_256_1 = nn.BatchNorm2d(lv1_c)
            self.conv_256_2 = nn.Conv2d(
                in_channels=lv1_c, out_channels=lv1_c, kernel_size=3, padding=1)
...
class HyperTransformerResBlock(nn.Module):
    ...
    def __init__(self, HSI_in_c, n_feates, lv, temperature):
        ...
        self.LFE_HSI = LFE_lvx(in_channels=self.HSI_in_c,
                               n_feates=n_feates, level=lv)
        self.LFE_PAN = LFE_lvx(in_channels=1,  n_feates=n_feates, level=lv)

        # Attention
        self.DotProductAttention = ScaledDotProductAttentionOnly(
            temperature=temperature)

        # Texture & Spectral Mixing
        self.TSmix = nn.Conv2d(in_channels=int(
            2*out_channels), out_channels=out_channels, kernel_size=3, padding=1)
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, in_pixels, linear_dim, num_features):
        ...
        self.w_ks = nn.Linear(in_pixels, n_head * linear_dim,
                              bias=False)  # Linear layer for keys
        # Linear layer for values
        self.w_vs = nn.Linear(in_pixels, n_head * linear_dim, bias=False)
        self.fc = nn.Linear(n_head * linear_dim, in_pixels,
                            bias=False)  # Final fully connected layer

        # Scaled dot product attention
        self.attention = ScaledDotProductAttention(
            temperature=in_pixels ** 0.5)
    ...
    def forward(self, v, k, q, mask=None):
        ...
        v_attn = v_attn.transpose(1, 2).contiguous().view(
            b, h*w, n_head * linear_dim)
...
class HyperTransformer(nn.Module):
    def __init__(self, lr_shape, **kwargs):
        super(HyperTransformer, self).__init__()
        # Settings
        self.is_DHP_MS = False
        self.in_channels = 4  # FIXME
        self.out_channels = 4  # FIXME
        self.factor = 4
        ...
        lv1_dim = lr_shape**2
        lv2_dim = (2*lr_shape)**2
        lv3_dim = (4*lr_shape)**2
        ...
        if n_head == 0:
            ...
            lv1_pixels = lr_shape**2
            lv2_pixels = (2*lr_shape)**2
            lv3_pixels = (4*lr_shape)**2
        ...
        self.SFE = SFE(self.in_channels,
                       self.num_res_blocks[0], self.n_feats, self.res_scale)
        ...
        ###############
        if False:
            self.conv22_headSUM = conv3x3(
                int(self.n_feats/2), int(self.n_feats/2))
        else:
            self.conv22_head = conv3x3(
                2*int(self.n_feats/2), int(self.n_feats/2))
        ...
        for i in range(self.num_res_blocks[2]):
            self.RB22.append(ResBlock(in_channels=int(
                self.n_feats/2), out_channels=int(self.n_feats/2), res_scale=self.res_scale))
        ...
        ###############
        if False:
            self.conv33_headSUM = conv3x3(
                int(self.n_feats/4), int(self.n_feats/4))
        else:
            self.conv33_head = conv3x3(
                2*int(self.n_feats/4), int(self.n_feats/4))
        self.RB33 = nn.ModuleList()
        for i in range(self.num_res_blocks[3]):
            self.RB33.append(ResBlock(in_channels=int(
                self.n_feats/4), out_channels=int(self.n_feats/4), res_scale=self.res_scale))
        self.conv33_tail = conv3x3(int(self.n_feats/4), int(self.n_feats/4))

        ##############
        ### FINAL ####
        ##############
        self.final_conv = nn.Conv2d(in_channels=self.n_feats+int(self.n_feats/2)+int(
            self.n_feats/4), out_channels=self.out_channels, kernel_size=3, padding=1)
        self.RBF = nn.ModuleList()
        for i in range(self.num_res_blocks[4]):
            self.RBF.append(ResBlock(in_channels=self.out_channels,
                            out_channels=self.out_channels, res_scale=self.res_scale))
        ...
        # MUlti-Scale-Output #
        ######################
        self.up_conv13 = nn.ConvTranspose2d(
            in_channels=self.n_feats, out_channels=self.in_channels, kernel_size=3, stride=4, output_padding=1)
        self.up_conv23 = nn.ConvTranspose2d(in_channels=int(
            self.n_feats/2), out_channels=self.in_channels, kernel_size=3, stride=2, padding=1, output_padding=1)

        ###########################
        # Transfer Periferal Loss #
        ###########################
        self.VGG_LFE_HSI = VGG_LFE(
            in_channels=self.in_channels, requires_grad=False)
        self.VGG_LFE_PAN = VGG_LFE(in_channels=1, requires_grad=False)

    def forward(self, X_PAN, X_MS):
        with torch.no_grad():
            if not self.is_DHP_MS:
                X_MS_UP = F.interpolate(X_MS, scale_factor=(
                    self.factor, self.factor), mode='bicubic')
            ...
            PAN_D = F.interpolate(X_PAN, scale_factor=(
                1/self.factor, 1/self.factor), mode='bilinear')
            PAN_UD = F.interpolate(PAN_D, scale_factor=(
                self.factor, self.factor), mode='bilinear')
        ...
        if False:
            ...
            # F.relu(self.conv11_head(x11_res))
            x11_res = self.conv11_headSUM(x11_res)
        else:
        ...
        if False:
            ...
            x22_res = self.conv22_headSUM(x22_res)
        else:
        ...
        if False:
            ...
            # F.relu(self.conv33_head(x33_res))
            x33_res = self.conv33_headSUM(x33_res)
        else:
        ...
        loss_tp = LOSS_TP(Phi_T_lv1, T_lv1)+LOSS_TP(Phi_T_lv2,
                                                    T_lv2)+LOSS_TP(Phi_T_lv3, T_lv3)
...
if __name__ == "__main__":
    shape = 16
    hypertransformer = HyperTransformer(shape)
    shape = shape * 4
    lr = torch.randn(8, 4, shape, shape)
    pan = torch.rand(8, 1, shape * 4, shape * 4)

    print(hypertransformer(pan, lr).shape)
